<template>
  <div class="q-pa-md" style="max-width: 300px">
    <q-form class="q-gutter-md" @reset="onReset" @submit="register">
      <q-input ref="emailRef" v-model="email" :rules="[ val => val && val.length > 3 || 'Please type email']" filled hint="Email"></q-input>
      ...
      <div class="q-pa-md">
        <router-link to="/login">Alredy have an account? Login.</router-link>
      </div>

      <div>
        <q-btn color="primary" label="Register" type="submit"></q-btn>
...
export default {
  ...
  methods: {
    register() {
      ...
      ImagesService.register(credentials)
        .then(({ data }) => {
